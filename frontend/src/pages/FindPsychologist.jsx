...
export default function FindPsychologist() {
    ...
    const fetchData = async (currentSpec = spec) => {
        setLoading(true);
        try {
            const { data } = await getPsychologists({ search, specialization: currentSpec });
            setPsychologists(data.psychologists || []);
        } catch (e) { console.error(e); }
        finally { setLoading(false); }
    };

    useEffect(() => { fetchData(spec); }, [spec]);
                    ...
                    <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginBottom: 18 }}>
                        <div style={{ flex: 1, minWidth: 220, position: 'relative' }}>
                            ...
                            <input
                                ...
                                value={search}
                                onChange={e => setSearch(e.target.value)}
                                onKeyDown={e => e.key === 'Enter' && fetchData(spec)}
                            />
                        </div>
                        <button className="search-btn" onClick={() => fetchData(spec)}>Search</button>
                    </div>

                    <div style={{ display: 'flex', gap: 7, flexWrap: 'wrap' }}>
                        <button
                            className={`spec-btn${spec === '' ? ' active' : ''}`}
                            onClick={() => { setSpec(''); fetchData(''); }}>
                            All
                        </button>
                        {SPECIALIZATIONS.map(s => (
                            <button
                                key={s}
                                className={`spec-btn${spec === s ? ' active' : ''}`}
                                onClick={() => { setSpec(s); fetchData(s); }}>
                                {s}
                            </button>
                        ))}
                    </div>
                                ...
                                <div key={p._id} className="psy-card fade-up" style={{ animationDelay: `${i * 0.07}s` }}>
                                    ...
                                    <div style={{ display: 'flex', gap: 13, marginBottom: 16, alignItems: 'flex-start' }}>
                                        <div style={{
                                            ...
                                            display: 'flex', alignItems: 'center', justifyContent: 'center',
                                            fontFamily: 'Sora, sans-serif', fontWeight: 700, fontSize: 20,
                                            background: p.avatar ? 'transparent' : `linear-gradient(135deg, ${av.from}, ${av.to})`,
                                            color: av.color, overflow: 'hidden',
                                        }}>
                                            {p.avatar
                                                ? <img src={p.avatar} alt={p.name} style={{ width: '100%', height: '100%', borderRadius: '50%', objectFit: 'cover' }} onError={e => { e.target.style.display = 'none'; }} />
                                                : initials}
                                        </div>
                                        <div style={{ flex: 1 }}>
                                            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 3 }}>
