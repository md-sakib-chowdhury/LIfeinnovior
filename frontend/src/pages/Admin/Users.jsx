...
const ROLES = ['all', 'superadmin', 'psychologist', 'patient'];
const SPECIALIZATIONS = ['Anxiety', 'Depression', 'Trauma', 'PTSD', 'OCD', 'ADHD', 'Bipolar', 'Relationships', 'Grief', 'Stress', 'Addiction', 'Sleep Disorders'];

const defaultForm = {
    name: '', email: '', password: '', role: 'patient', phone: '',
    avatar: '', bio: '', sessionFee: '', experience: '', specializations: []
};
...
export default function AdminUsers() {
    ...
    const openEdit = (user) => {
        ...
        setForm({
            ...
            sessionFee: user.psychologistProfile?.sessionFee || '',
            experience: user.psychologistProfile?.experience || '',
            specializations: user.psychologistProfile?.specializations || []
        });
        setModal(true);
    };
    ...
    const toggleSpec = (s) => {
        ...
        setForm(p => ({ ...p, specializations: cur.includes(s) ? cur.filter(x => x !== s) : [...cur, s] }));
    };
        ...
        try {
            if (editing) {
                const payload = {
                    ...
                    phone: form.phone,
                    avatar: form.avatar,
                    psychologistProfile: form.role === 'psychologist' ? {
        ...
        <div style={{ display: 'flex', minHeight: '100vh', background: '#F8FAFF' }}>
                ...
                <div style={{ padding: 28 }}>
                    ...
                    <div style={{ display: 'flex', gap: 12, marginBottom: 20, flexWrap: 'wrap' }}>
                        ...
                        <div style={{ display: 'flex', gap: 6 }}>
                            {ROLES.map(r => (
                                <button key={r} onClick={() => { setRole(r); setPage(1); }}
                                    style={{ padding: '8px 16px', borderRadius: 10, cursor: 'pointer', fontWeight: 500, fontSize: 13, background: role === r ? 'linear-gradient(135deg, #4F46E5, #7C3AED)' : '#fff', color: role === r ? '#fff' : '#6B7280', border: role === r ? 'none' : '1px solid #E5E7EB', boxShadow: role === r ? '0 2px 8px rgba(79,70,229,0.3)' : 'none' }}>
                                    {r.charAt(0).toUpperCase() + r.slice(1)}
                                </button>
                            ))}
                        </div>
                                        ...
                                        <tr key={user._id} style={{ borderBottom: '1px solid #F3F4F6' }}
                                            ...
                                            <td style={{ padding: '16px 20px' }}>
                                                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                                                    <div style={{ width: 40, height: 40, borderRadius: '50%', background: user.avatar ? 'transparent' : 'linear-gradient(135deg, #4F46E5, #7C3AED)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, color: '#fff', fontSize: 16, flexShrink: 0, overflow: 'hidden' }}>
                                                        {user.avatar ? <img src={user.avatar} alt={user.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : user.name?.charAt(0)?.toUpperCase()}
                                                    </div>
                                                    <div>
            ...
            {modal && (
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 20 }} onClick={() => setModal(false)}>
                    <div style={{ background: '#fff', borderRadius: 20, padding: 32, width: '100%', maxWidth: 600, maxHeight: '90vh', overflowY: 'auto' }} onClick={e => e.stopPropagation()}>
                        ...
                        <form onSubmit={handleSave}>
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                                ...
                                <div>
                                    ...
                                    <input style={{ width: '100%', padding: '10px 14px', border: '1px solid #E5E7EB', borderRadius: 10, fontSize: 14, outline: 'none', fontFamily: 'inherit', background: editing ? '#F9FAFB' : '#fff' }}
                                ...
                                {/* Profile Image */}
                                <div>
                                    <label style={{ display: 'block', fontSize: 13, fontWeight: 600, marginBottom: 6, color: '#374151' }}>Profile Image URL</label>
                                    <input style={{ width: '100%', padding: '10px 14px', border: '1px solid #E5E7EB', borderRadius: 10, fontSize: 14, outline: 'none', fontFamily: 'inherit' }}
                                        type="url" value={form.avatar} onChange={e => setForm(p => ({ ...p, avatar: e.target.value }))} placeholder="https://example.com/photo.jpg" />
                                    {form.avatar && (
                                        <div style={{ marginTop: 10, display: 'flex', alignItems: 'center', gap: 10 }}>
                                            <img src={form.avatar} alt="Preview" style={{ width: 48, height: 48, borderRadius: '50%', objectFit: 'cover', border: '2px solid #E5E7EB' }}
                                                onError={e => e.target.style.display = 'none'} />
                                            <span style={{ fontSize: 12, color: '#6B7280' }}>Image preview</span>
                                        </div>
                                    )}
                                </div>
                                ...
                                {form.role === 'psychologist' && (
                                    <>
                                        <div style={{ padding: 16, background: '#F5F3FF', borderRadius: 12, border: '1px solid #DDD6FE' }}>
                                            <div style={{ fontSize: 13, fontWeight: 700, color: '#7C3AED', marginBottom: 12 }}>👨‍⚕️ Psychologist Details</div>

                                            <div style={{ marginBottom: 12 }}>
                                                <label style={{ display: 'block', fontSize: 13, fontWeight: 600, marginBottom: 8, color: '#374151' }}>Specializations</label>
                                                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                                                    {SPECIALIZATIONS.map(s => (
                                                        <button key={s} type="button" onClick={() => toggleSpec(s)}
                                                            style={{ padding: '5px 12px', borderRadius: 20, border: 'none', cursor: 'pointer', fontSize: 12, fontWeight: 500, transition: 'all 0.15s', background: form.specializations.includes(s) ? 'linear-gradient(135deg, #7C3AED, #4F46E5)' : '#fff', color: form.specializations.includes(s) ? '#fff' : '#6B7280', border: form.specializations.includes(s) ? 'none' : '1px solid #DDD6FE', boxShadow: form.specializations.includes(s) ? '0 2px 8px rgba(124,58,237,0.3)' : 'none' }}>
                                                            {s}
                                                        </button>
                                                    ))}
                                                </div>
                                            ...
                                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 12 }}>
                                                <div>
                                                    <label style={{ display: 'block', fontSize: 13, fontWeight: 600, marginBottom: 6, color: '#374151' }}>Experience (years)</label>
                                                    <input style={{ width: '100%', padding: '10px 14px', border: '1px solid #DDD6FE', borderRadius: 10, fontSize: 14, outline: 'none', fontFamily: 'inherit', background: '#fff' }}
                                                        type="number" min="0" value={form.experience} onChange={e => setForm(p => ({ ...p, experience: e.target.value }))} placeholder="e.g. 5" />
                                                </div>
                                                <div>
                                                    <label style={{ display: 'block', fontSize: 13, fontWeight: 600, marginBottom: 6, color: '#374151' }}>Session Fee ($)</label>
                                                    <input style={{ width: '100%', padding: '10px 14px', border: '1px solid #DDD6FE', borderRadius: 10, fontSize: 14, outline: 'none', fontFamily: 'inherit', background: '#fff' }}
                                                        type="number" min="0" value={form.sessionFee} onChange={e => setForm(p => ({ ...p, sessionFee: e.target.value }))} placeholder="e.g. 50" />
                                                </div>
                                            </div>

                                            <div>
                                                <label style={{ display: 'block', fontSize: 13, fontWeight: 600, marginBottom: 6, color: '#374151' }}>Professional Bio</label>
                                                <textarea style={{ width: '100%', padding: '10px 14px', border: '1px solid #DDD6FE', borderRadius: 10, fontSize: 14, outline: 'none', fontFamily: 'inherit', resize: 'vertical', minHeight: 80, background: '#fff' }}
                                                    value={form.bio} onChange={e => setForm(p => ({ ...p, bio: e.target.value }))} placeholder="Brief professional bio..." />
                                            </div>
                                        </div>
                                    </>
                                )}
                            ...
                            <div style={{ display: 'flex', gap: 12, justifyContent: 'flex-end', marginTop: 24 }}>
                                <button type="button" onClick={() => setModal(false)}
                                    style={{ padding: '10px 20px', border: '1px solid #E5E7EB', borderRadius: 10, background: '#fff', cursor: 'pointer', fontWeight: 500, fontSize: 14 }}>
                                    Cancel
                                </button>
                                <button type="submit" disabled={saving}
                                    style={{ padding: '10px 24px', background: 'linear-gradient(135deg, #4F46E5, #7C3AED)', color: '#fff', border: 'none', borderRadius: 10, cursor: 'pointer', fontWeight: 600, fontSize: 14, boxShadow: '0 4px 12px rgba(79,70,229,0.3)', opacity: saving ? 0.7 : 1 }}>
